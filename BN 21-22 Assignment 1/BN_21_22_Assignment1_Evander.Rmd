...
#library(xtable)
#print(xtable(top_rmsea[1:12,1:4 ], type = "latex"), file = "filename2.tex")
...
corrtest <- localTests(g1, sample.cov = d2_proc_corr, sample.nobs=nrow(d2_proc))
plotLocalTestResults(corrtest)
top_corr <- corrtest[order(corrtest$p.value,decreasing = FALSE),]
print(top_corr)
...
semPaths(fit, what="est", whatLabels = "par", style = "OpenMx", layout = "tree2", residuals = FALSE, nCharNodes=0, edge.label.cex = 1.5, asize = 6, minimum = 0.1, sizeMan = 12,sizeMan2 = 5,curvature = 1.5, rotation=1,curve=2, exoCov=FALSE)
